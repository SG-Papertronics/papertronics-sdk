...
class LabClient:
    ...
    def __init__(self, url):  # TODO: set query input to httpx params
        self.url = url
        self.token = None

    def authenticate(self, email, password):
        response = httpx.post(f"{self.url}/auth/token", data=dict(username=email, password=password))
        self.token = response.json().get("access_token")
        return self.token

    def authenticate_device(self, device_id: uuid.UUID):
        response = httpx.post(f"{self.url}/auth/device?device_id={device_id}",
    ...
    def get_protocol(self,
        ...
        response = httpx.get(f"{self.url}/protocol",
                             headers={"Authorization": f"Bearer {self.token}"},
    ...
    def count_protocol(self,
        ...
        response = httpx.get(f"{self.url}/protocol/count",
                             headers={"Authorization": f"Bearer {self.token}"},
    ...
    def get_protocol_test_def(self, protocol_id: uuid.UUID) -> Union[ProtocolTestResponse, str]:
        response = httpx.get(f"{self.url}/protocol/test?protocol_id={protocol_id}",
                             headers={"Authorization": f"Bearer {self.token}"})
    ...
    def invalidate_protocol(self, protocol_id: uuid.UUID) -> ProtocolModel:
        response = httpx.delete(f"{self.url}/protocol?protocol_id={protocol_id}",
                                headers={"Authorization": f"Bearer {self.token}"})
        return ProtocolModel.parse_obj(response.json())

    async def start_experiment(self, request: ExperimentStartRequest) -> ExperimentModel:
        response = httpx.post(f"{self.url}/experiment/start",
                              headers={"Authorization": f"Bearer {self.token}"},
                              json=request.dict())
    ...
    def cancel_experiment(self,
        ...
        httpx.post(f"{self.url}/experiment/cancel",
                   headers={"Authorization": f"Bearer {self.token}"},
    ...
    def get_experiments(self,
        ...
        response = httpx.get(f"{self.url}/experiment",
                             headers={"Authorization": f"Bearer {self.token}"},
    ...
    def count_experiments(self,
        ...
        response = httpx.get(f"{self.url}/experiment/count",
                             headers={"Authorization": f"Bearer {self.token}"},
    ...
    def edit_experiment_by_id(self,
        ...
        response = httpx.post(f"{self.url}/experiment/edit?experiment_id={experiment_id}",
                              headers={"Authorization": f"Bearer {self.token}"},
    ...
    def delete_experiment_by_id(self,
                                experiment_id: uuid.UUID):
        httpx.delete(f"{self.url}/experiment?experiment_id={experiment_id}",
                     headers={"Authorization": f"Bearer {self.token}"})
